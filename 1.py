...
class CardTrackerApp:
    def __init__(self, root):
        ...
        # Заголовок
        tk.Label(root, text="Карты", font=("Georgia", 26, "italic", "bold")).pack(pady=10)
        ...
        # Кнопка обновления
        tk.Button(root, text="Сбросить и обновить список", font=("Courier", 16, "italic"), command=self.reset_and_update).pack(pady=10)
    ...
    def update_card_list(self):
        ...
        for suit, cards in cards_by_suit.items():
            ...
            for card_id, rank, is_played in cards:
                ...
                text_color = "red" if suit in ["Черви", "Бубны"] else "black"

                # Учитываем выбывшие карты
                text = f"{rank}" if not is_played else f"~{rank}~"
                faded_color = "gray" if is_played else text_color

                # Создаем текстовую метку для карты
                tk.Label(
                    self.card_frame,
                    text=text,
                    font=("Arial", 10),
                    fg=faded_color  # Цвет текста
    ...
    def update_card_list(self):
        ...
        for suit, cards in cards_by_suit.items():
            ...
            tk.Label(self.card_frame, text=f"{suit}:", font=("Comic Sans MS", 22, "italic")).grid(row=row, column=0, padx=10,
                                                                                        pady=5, sticky="w")

            col = 1
            for card_id, rank, is_played in cards:
                image_path = f"{IMAGE_PATH}/{rank.lower()}_{suit.lower()}.png"
                fg_color = "red" if suit in ["Черви", "Бубны"] else "black"
                try:
                    image = Image.open(image_path)
                    if is_played:
                        image = create_faded_cross_image(image)
                    photo = ImageTk.PhotoImage(image.resize((80, 120)))
